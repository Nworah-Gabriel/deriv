import { OIDCError as m, OIDCErrorType as S } from "./error.js";
...
}, E, T, J = /* @__PURE__ */ ((e) => (e[e.NONE = 0] = "NONE", e[e.ERROR = 1] = "ERROR", e[e.WARN = 2] = "WARN", e[e.INFO = 3] = "INFO", e[e.DEBUG = 4] = "DEBUG", e))(J || {});
((e) => {
  function t() {
    E = 3, T = Ne;
  }
...
var h = class b {
  ...
  debug(...t) {
    E >= 4 && T.debug(b._format(this._name, this._method), ...t);
  }
  info(...t) {
    E >= 3 && T.info(b._format(this._name, this._method), ...t);
  }
  warn(...t) {
    E >= 2 && T.warn(b._format(this._name, this._method), ...t);
  }
  error(...t) {
    E >= 1 && T.error(b._format(this._name, this._method), ...t);
  }
  ...
  static debug(t, ...s) {
    E >= 4 && T.debug(b._format(t), ...s);
  }
  static info(t, ...s) {
    E >= 3 && T.info(b._format(t), ...s);
  }
  static warn(t, ...s) {
    E >= 2 && T.warn(b._format(t), ...s);
  }
  static error(t, ...s) {
    E >= 1 && T.error(b._format(t), ...s);
  }
  /* eslint-enable @typescript-eslint/no-unsafe-enum-comparison */
};
J.reset();
var L = class {
...
}, Me = "10000000-1000-4000-8000-100000000000", B = (e) => btoa([...new Uint8Array(e)].map((t) => String.fromCharCode(t)).join("")), de = class k {
  static _randomWord() {
  ...
  static generateUUIDv4() {
    return Me.replace(
      /[018]/g,
      (s) => (+s ^ k._randomWord() & 15 >> +s / 4).toString(16)
    ).replace(/-/g, "");
  }
  /**
  ...
  static async customCalculateJwkThumbprint(t) {
    ...
    const i = await k.hash("SHA-256", JSON.stringify(s));
    return k.encodeBase64Url(i);
  }
  static async generateDPoPProof({
    ...
    s && (n = await k.hash("SHA-256", s), a = k.encodeBase64Url(n), c.ath = a), o && (c.nonce = o);
    try {
  ...
  static async generateDPoPJkt(t) {
    try {
      ...
      return await k.customCalculateJwkThumbprint(s);
    } catch (s) {
...
var f = de, P = class {
  ...
  async postForm(e, {
    ...
    if (!c.ok) {
      if (n.error("Error from server:", g), c.headers.has("dpop-nonce")) {
        ...
        throw new Q(u, `${JSON.stringify(g)}`);
      }
...
}, $e = "code", Je = "openid", Le = "client_secret_post", Fe = 60 * 15, X = class {
  constructor({
    // metadata related
    authority: e,
    metadataUrl: t,
  ...
  static async create({
    ...
    let I = v.id;
    u && (I = `${I}${G}${u}`), _.searchParams.append("state", I), v.code_challenge && (_.searchParams.append("code_challenge", v.code_challenge), _.searchParams.append("code_challenge_method", "S256")), p && (Array.isArray(p) ? p : [p]).forEach((w) => _.searchParams.append("resource", w));
    for (const [y, w] of Object.entries({ response_mode: c, ...N, ...O }))
      w != null && _.searchParams.append(y, w.toString());
    return new pe({
      url: _.href,
      state: v
    });
...
var ze = _e, Be = "openid", K = class {
  ...
  constructor(e, t) {
    this._logger = new h("OidcClient"), this.settings = e instanceof X ? e : new X(e), this.metadataService = t ?? new He(this.settings), this._claimsService = new Ye(this.settings), this._validator = new Ke(this.settings, this.metadataService, this._claimsService), this._tokenClient = new he(this.settings, this.metadataService);
  }
  ...
  async processSigninResponse(e, t) {
    ...
    try {
      ...
      if (o instanceof Q && this.settings.dpop) {
  ...
  async useRefreshToken({
    ...
    try {
      ...
      if (g instanceof Q && this.settings.dpop)
  ...
  async readSignoutResponseState(e, t = !1) {
    const s = this._logger.create("readSignoutResponseState"), i = new Ge(V.readParams(e, this.settings.response_mode));
    if (!i.state) {
  ...
  constructor(e) {
    ...
    if (super(e), this.popup_redirect_uri = t, this.popup_post_logout_redirect_uri = s, this.popupWindowFeatures = i, this.popupWindowTarget = r, this.redirectMethod = o, this.redirectTarget = n, this.iframeNotifyParentOrigin = a, this.iframeScriptOrigin = c, this.silent_redirect_uri = d, this.silentRequestTimeoutInSeconds = g || l || ye, this.automaticSilentRenew = u, this.validateSubOnSilentRenew = p, this.includeIdTokenInSilentRenew = x, this.monitorSession = O, this.monitorAnonymousSession = A, this.checkSessionIntervalInSeconds = q, this.stopCheckSessionOnError = C, this.query_status_response_type = U, this.revokeTokenTypes = N, this.revokeTokensOnSignout = v, this.includeIdTokenInSilentSignout = _, this.accessTokenExpiringNotificationTimeInSeconds = I, y)
      ...
      this.userStore = new se({ store: w });
    }
  }
}, ae = class be extends me {
  constructor({
    silentRequestTimeoutInSeconds: t = ye
  }) {
    super(), this._logger = new h("IFrameWindow"), this._timeoutInSeconds = t, this._frame = be.createHiddenIframe(), this._window = this._frame.contentWindow;
  }
...
const ut = async () => {
  ...
  try {
    ...
    throw console.error("Failed to fetch OIDC configuration:", i), i instanceof Error ? new m(S.FailedToFetchOIDCConfiguration, i.message) : new m(S.FailedToFetchOIDCConfiguration, "unable to fetch OIDC configuration");
  }
}, St = async (e) => {
  ...
  try {
    ...
    return await r.signinRedirect({
      extraQueryParams: {
        brand: "deriv"
      }
    }), { userManager: r };
  } catch (r) {
    throw console.error("Authentication failed:", r), r instanceof Error ? new m(S.AuthenticationRequestFailed, r.message) : new m(S.AuthenticationRequestFailed, "unable to request OIDC authentication");
  }
}, vt = async (e) => {
  const { redirectCallbackUri: t, postLogoutRedirectUri: s } = e;
  try {
    const r = await (await W({
      redirectCallbackUri: t,
    ...
    throw console.error("unable to request access tokens: ", i), i instanceof Error ? new m(S.AccessTokenRequestFailed, i.message) : new m(S.AccessTokenRequestFailed, "unable to request access tokens");
  }
}, yt = async (e) => {
  const { serverUrl: t } = D();
  try {
    ...
    throw console.error("unable to request legacy tokens: ", s), s instanceof Error ? new m(S.LegacyTokenRequestFailed, s.message) : new m(S.LegacyTokenRequestFailed, "unable to request legacy tokens");
  }
}, W = async (e) => {
  ...
  try {
    ...
    throw console.error("unable to create user manager for OIDC: ", a), a instanceof Error ? new m(S.UserManagerCreationFailed, a.message) : new m(S.UserManagerCreationFailed, "unable to create user manager for OIDC");
  }
}, bt = async (e) => {
  ...
  const i = () => {
    ...
    if (n.data === "logout_complete") {
      ...
      a.includes(c) && le.set("logged_state", "false", {
        ...
        domain: c,
        secure: !0
      }), e.WSLogoutAndRedirect(), i(), window.removeEventListener("message", r);
    }
  };
  ...
  try {
    ...
    throw t instanceof Error ? new m(S.FailedToRemoveSession, t.message) : new m(S.FailedToRemoveSession, "unable to remove OIDC session");
  }
}, Et = (e) => {
...
export {
  ...
  W as createUserManager,
  ut as fetchOidcConfiguration,
  Et as handlePostLogout,
  kt as oidcLogout,
  yt as requestLegacyToken,
  St as requestOidcAuthentication,
  vt as requestOidcToken
};
